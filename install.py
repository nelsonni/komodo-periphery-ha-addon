...
class Installer:
    ...
    def print_header(self):
        ...
        print(f"\n{Colors.BLUE}{'=' * 46}")
        print(f"  Komodo Periphery HA Add-on Installer")
    ...
    def install_dependencies(self):
        ...
        missing_tools = [tool for tool in required_tools if not self.command_exists(tool)]
    ...
    def _install_linux_dependencies(self, missing_tools: List[str]):
        ...
        if pkg_manager == 'apt-get':
            ...
            self.run_command(['sudo', 'apt-get', 'install', '-y'] + missing_tools + ['docker-compose', 'jq'])
        elif pkg_manager in ['yum', 'dnf']:
            self.run_command(['sudo', pkg_manager, 'update', '-y'])
            self.run_command(['sudo', pkg_manager, 'install', '-y'] + missing_tools + ['docker-compose', 'jq'])
        elif pkg_manager == 'pacman':
            self.run_command(['sudo', 'pacman', '-Syu', '--noconfirm'] + missing_tools + ['docker-compose', 'jq'])
        ...
        if 'docker' in missing_tools:
            try:
                ...
                self.run_command(['sudo', 'usermod', '-aG', 'docker', os.getenv('USER', 'user')])
                self.print_warning("You may need to log out and back in for Docker group changes to take effect.")
            except subprocess.CalledProcessError:
                self.print_warning("Could not configure Docker service. Please configure manually.")
    ...
    def _install_macos_dependencies(self, missing_tools: List[str]):
        ...
        if not self.command_exists('docker'):
            self.print_warning("Docker Desktop not found. Please install Docker Desktop for Mac:")
    ...
    def _check_windows_dependencies(self, missing_tools: List[str]):
        ...
        if missing_tools:
            ...
            self.print_warning("- Docker Desktop: https://docs.docker.com/desktop/install/windows-install/")
    ...
    def find_ha_config(self):
        ...
        for path in ha_paths:
            if (path / 'configuration.yaml').exists():
                ...
                self.print_status(f"Found Home Assistant config at: {self.ha_config_dir}")
                return

        # Ask user for path
        self.print_warning("Home Assistant configuration directory not found automatically.")
        user_path = input("Please enter the path to your Home Assistant config directory: ")
    ...
    def build_addon(self):
        ...
        if not self.command_exists('docker'):
            self.print_error("Docker not available. Please install Docker to build the addon.")
            sys.exit(1)

        try:
            self.print_status(f"Building Docker image: {image_name}")
            self.run_command(['docker', 'build', '--build-arg', f'BUILD_ARCH={arch}', '-t', image_name, '.'])
...
"""
    ...
    def create_documentation(self):
        ...
        dev_doc = f"""# Komodo Periphery Add-on Development
...
"""
            ...
            self.run_command(['git', 'commit', '-m', 'Initial commit: Komodo Periphery Home Assistant Add-on'])
...
def main():
    ...
    try:
        ...
        if args.dev:
            ...
            print("4. Find the add-on in Home Assistant Supervisor -> Add-on Store -> Local add-ons")
