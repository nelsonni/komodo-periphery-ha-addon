    ...
    def test_ports_configuration(self, config_data: Dict[str, Any]):
        ...
        if ports:
            ...
            for port_def, port_num in ports.items():
                ...
                # Test port number
                assert isinstance(port_num, int), f"Port number must be integer: {port_num}"
    ...
    def test_schema_types(self, config_data: Dict[str, Any]):
        ...
        for field, field_type in schema.items():
            if isinstance(field_type, str):
                ...
                if '|' in base_type:  # Handle enum types
                    continue
        ...
        if services:
            ...
            for service in services:
                ...
                if ':' in service:
                    ...
                    assert dependency_type in ['want', 'need'], f"Invalid dependency type: {dependency_type}"
...
class TestBuildConfig:
    ...
    def test_build_config_is_valid_yaml(self, build_config_data: Dict[str, Any]):
        ...
        assert isinstance(build_config_data, dict), "Build configuration must be a dictionary"
    ...
    def test_build_from_configuration(self, build_config_data: Dict[str, Any], config_data: Dict[str, Any]):
        ...
        for arch in supported_archs:
            ...
            assert isinstance(base_image, str), f"Base image for {arch} must be a string"
    ...
    def test_build_args(self, build_config_data: Dict[str, Any]):
        ...
        if args:
            ...
            if 'KOMODO_VERSION' in args:
                assert isinstance(args['KOMODO_VERSION'], str), "KOMODO_VERSION must be a string"
    ...
    def test_labels_configuration(self, build_config_data: Dict[str, Any]):
        ...
        if labels:
            ...
            for label in required_labels:
                if label in labels:
                    assert isinstance(labels[label], str), f"Label {label} must be a string"
...
class TestTranslations:
    ...
    def test_translation_structure(self, project_root_path: Path, config_data: Dict[str, Any]):
        ...
        for option_key in options.keys():
            ...
            assert isinstance(option_translation['name'], str), f"Translation name for {option_key} must be string"
            assert isinstance(option_translation['description'], str), f"Translation description for {option_key} must be string"
            assert len(option_translation['name']) > 0, f"Translation name for {option_key} must not be empty"
            assert len(option_translation['description']) > 0, f"Translation description for {option_key} must not be empty"